...
NTSTATUS driver_entry( uintptr_t magic_key, PDRIVER_OBJECT driver_object )
{
	...
	auto ntoskrnl = fusion::winapi::get_module_handle<void*>( ( "ntoskrnl.exe" ) );
	TRACE( "ntoskrnl: 0x%llx", ntoskrnl );

	auto function = fusion::winapi::find_pattern<uintptr_t>( ntoskrnl,
		"\x4C\x8B\x05\x00\x00\x00\x00\x33\xC0\x4D\x85\xC0\x74\x08\x49\x8B\xC0\xE8\x00\x00\x00\x00\xF7\xD8", "xxx????xxxxxxxxxxx????xx" );
	if ( !function )
	{
