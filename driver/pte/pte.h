...
namespace fusion::pte
{
    ...
    uintptr_t allocate( size_t size )
    {
        ...
        if ( NT_SUCCESS( ZwAllocateVirtualMemory( ( ( HANDLE ) ( LONG_PTR ) -1 ), reinterpret_cast< PVOID* >( &allocated ), 0, &size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE ) ) )
        {
            for ( uintptr_t i = allocated; i < ( ( allocated + size ) & 0xfffffffffffff000 ); i += 0x1000 )
            {
               ...
               if ( ( pte = get_pte_address( i ) ) )
                {
                    TRACE( "0x%llx | pte->m_no_execute = %d", i, pte->m_no_execute );
                    pte->m_no_execute = 0;
                    TRACE( "0x%llx | pte->m_no_execute = %d", i, pte->m_no_execute );
                }
                else
