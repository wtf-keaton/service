...
namespace fusion::syscall
{
	...
	__forceinline bool nt_create_thread( HANDLE handle, PVOID address )
	{
		HANDLE thread_handle = NULL;
		...
		auto status = sys_call<NTSTATUS, PHANDLE, ACCESS_MASK, POBJECT_ATTRIBUTES, HANDLE, PTHREAD_START_ROUTINE, ULONG, ULONG_PTR, SIZE_T, SIZE_T, PVOID>(
			HASH( "NtCreateThreadEx" ),
			&thread_handle, THREAD_ALL_ACCESS,
			NULL, handle, reinterpret_cast< PTHREAD_START_ROUTINE >( address ),
			NULL, 0x00000004, NULL, NULL, NULL, nullptr );
	...
	__forceinline bool read( HANDLE handle, uintptr_t addr, void* data, size_t size )
	{
		auto status = sys_call<NTSTATUS, HANDLE, void*, void*, size_t, size_t*>( HASH( "NtReadVirtualMemory" ), handle, reinterpret_cast< void* >( addr ), data, size, nullptr );
		if ( !NT_SUCCESS( status ) )
			return false;
	...
	__forceinline bool write( HANDLE handle, uintptr_t addr, void* data, size_t size )
	{
		auto status = sys_call<NTSTATUS, HANDLE, void*, void*, size_t, size_t*>( HASH( "NtWriteVirtualMemory" ), handle, reinterpret_cast< void* >( addr ), data, size, nullptr );
		if ( !NT_SUCCESS( status ) )
			return false;
	...
	__forceinline uintptr_t allocate( HANDLE handle, size_t size, uint32_t type, uint32_t protection )
	{
		void* base{};
		auto status = sys_call<NTSTATUS, HANDLE, void**, ULONG_PTR, size_t*, ULONG, ULONG >( HASH( "NtAllocateVirtualMemory" ), handle, &base, 0, &size, type, protection );
	...
	__forceinline bool free( HANDLE handle, uintptr_t addre, size_t size, ULONG type )
	{
		auto addr_casted = reinterpret_cast< void* >( addre );
		auto status = sys_call<NTSTATUS, HANDLE, void**, size_t*, ULONG>( HASH( "NtFreeVirtualMemory" ), handle, &addr_casted, &size, type );
		if ( !NT_SUCCESS( status ) )
			return false;
	...
	__forceinline NTSTATUS suspend( HANDLE handle )
	{
		return syscall::sys_call<NTSTATUS, HANDLE, ULONG*>( HASH( "NtSuspendThread" ), handle, nullptr );
	}

	__forceinline NTSTATUS resume( HANDLE handle )
	{
		return syscall::sys_call<NTSTATUS, HANDLE, ULONG*>( HASH( "NtResumeThread" ), handle, nullptr );

	}

	__forceinline NTSTATUS get_ctx( HANDLE handle, CONTEXT* out )
	{
		return syscall::sys_call<NTSTATUS, HANDLE, CONTEXT*>( HASH( "NtGetContextThread" ), handle, out );
	}

	__forceinline NTSTATUS set_ctx( HANDLE handle, CONTEXT* ctx )
	{
		return syscall::sys_call<NTSTATUS, HANDLE, CONTEXT*>( HASH( "NtSetContextThread" ), handle, ctx );
	}
}
