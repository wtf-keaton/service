...
public:
	...
	T read( const std::uintptr_t address, void* buffer, std::size_t size )
	{
		operation request
		{
			address, //virtual address
	...
	std::uintptr_t module_base_address( const char* module_name ) const
	{
		operation request
		{
			0, // virtual address
		...
		return request.virtual_address;
	}

	uint32_t protect_memory( uint64_t address, size_t size, uint32_t protect, uint32_t old_protect ) {
		operation request
		{
			address, // virtual address
			size, // size
	...
	uint64_t alloc_memory( size_t size, uint32_t protect, uint64_t address = NULL ) {
		operation request
		{
			address, // virtual address
			size, // size
	...
	uint64_t alloc_memory_nx( size_t size, uint64_t address = NULL ) {
		operation request
		{
			address, // virtual address
			size, // size
	...
	NTSTATUS free_memory( uintptr_t address ) {
		operation request
		{
			address, // virtual address
			sizeof( request ), // size
