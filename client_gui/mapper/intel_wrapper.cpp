...
bool c_intel_wrapper::load( )
{
	...
	{
		strcpy_s( globals.error, _( "Error while loading driver #1" ) );
	...
	{
		strcpy_s( globals.error, _( "Error while loading driver #2" ) );
	...
	{
		strcpy_s( globals.error, _( "Error while loading driver #3" ) );
		DeleteFileW( m_full_path );
		RegCloseKey( reg_key );
		return false;
	}
	...
	{
		strcpy_s( globals.error, _( "Error while loading driver #4" ) );
	...
	{
		strcpy_s( globals.error, _( "Error while loading driver #5" ) );
	...
	if ( !m_driver || m_driver == INVALID_HANDLE_VALUE )
	{
		strcpy_s( globals.error, _( "Error while loading driver #6" ) );
		DeleteFileW( m_full_path );
		return false;
	}
